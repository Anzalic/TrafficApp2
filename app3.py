...
model = joblib.load('traffic_congestion_model.pkl')  # Replace with your actual model file path
data = pd.read_csv('cleaned_data_for_web_2.csv')  # Replace with your actual data file path
...
def get_filtered_roads(local_authority):
    if local_authority in data['local_authority_name'].values:
...
region_name = col3.selectbox("Select Region", ['London'], key='region_select')  # Assuming only 'London' is available
...
road_name = col2.selectbox("Road Name", filtered_road_names, key='road_name_select')
road_type = col3.selectbox("Road Type", list(road_type_mapping.keys()), key='road_type_select')
...
if st.button('Predict'):
    ...
    inputs = [
        ...
        road_type_mapping.get(road_type, -1)
    ]
